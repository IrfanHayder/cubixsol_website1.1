...
async function sync() {
  try {
    ...
    // Upsert / update each project by title
    let createdCount = 0;
    let updatedCount = 0;
    ...
    console.log(`✓ Sync complete: ${createdCount} created, ${updatedCount} updated. Total projects in MongoDB: ${totalInDb}`);

    // Update backend/seedData.json
    const seedJsonPath = path.join(__dirname, 'seedData.json');
    if (fs.existsSync(seedJsonPath)) {
      const seedData = JSON.parse(fs.readFileSync(seedJsonPath, 'utf8'));
      seedData.initialProjects = allProjects;
      fs.writeFileSync(seedJsonPath, JSON.stringify(seedData, null, 2), 'utf8');
      console.log('✓ Updated backend/seedData.json with 49 projects.');
    }
